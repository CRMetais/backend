...
""",
    ...
    @Query(value = """
        SELECT c.id_compra AS id, c.data_compra AS data, f.nome AS parceiro,
               p.nome AS produto, ipc.peso_kg AS peso, ipc.preco_unitario AS preco,
               (ipc.peso_kg * ipc.preco_unitario) AS total,
        ...
        """, nativeQuery = true)
    List<TransacaoHistoricoProjection> buscarCompras(
            @Param("dataInicio") LocalDate dataInicio,
            @Param("dataFim") LocalDate dataFim
    );

    @Query(value = """
        SELECT v.id_venda AS id, v.data_venda AS data, cl.razao_social AS parceiro,
               p.nome AS produto, ipv.peso_kg AS peso, ipv.preco_unitario AS preco,
               (ipv.peso_kg * ipv.preco_unitario) AS total,
        ...
        """, nativeQuery = true)
    List<TransacaoHistoricoProjection> buscarVendas(
            @Param("dataInicio") LocalDate dataInicio,
            @Param("dataFim") LocalDate dataFim
    );
}